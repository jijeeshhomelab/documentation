...
| ------- | ---------- | ---------------- | -------------|
| 0.1     | 27.02.2025 | First version    | Lukasz Tomaszewski |
...
Configure vSphere IaaS Control Plane (Tanzu) solution on management cluster.
...
- Create TKG Cluster for NAPP (additional operation)
...
NOTE: required packages were added to DHC-Versio-Matrix/DHC-2.0 branch, thus:
...
- tanzu.supervisor.management.kubernetesNetwork.ingressNetworkCidr
- tanzu.supervisor.management.kubernetesNetwork.egressNetworkCidr
...
tanzu:
  supervisor:
    management: #for vcs001
      apiServerDnsNames: [ "{{ locationCode }}tnz001.{{ searchDomain }}" ]
      vCenter: "{{ mgmtDns.vcs001.name }}.{{ searchDomain }}"
      name: "{{ locationCode }}-m01-supervisor01"
      clusterName: "{{ locationCode }}-m01-cluster01"
      zoneName: "{{ locationCode }}-m01-zone01"
      ...
      supervisorNetwork:
        ...
        startingIpAddress: "{{ networkMgmt.cidr }}.50"
        subnetMask: "{{ networkMgmt.netmask }}"
        gateway: "{{ networkMgmt.cidr }}.{{ networkMgmt.gw }}"
        dnsServers: [ "{{ mgmtDns.adc001.cidr }}.{{ mgmtDns.adc001.octet }}", "{{ mgmtDns.adc002.cidr }}.{{ mgmtDns.adc002.octet }}" ]
        dnsSearchDomains: [ "{{ searchDomain }}" ]
        ntpServers: [ "{{ mgmtDns.adc001.cidr }}.{{ mgmtDns.adc001.octet }}", "{{ mgmtDns.adc002.cidr }}.{{ mgmtDns.adc002.octet }}" ]
      kubernetesNetwork:
        vsphereDistributedSwitch:
        edgeCluster: "{{ locationCode }}ecn101"
        t0Gateway: "{{ locationCode }}ecn101-t0-gw01"
        t1Gateway: "{{ locationCode }}ecn101-t1-gw02"
        dnsServers: [ "{{ mgmtDns.adc001.cidr }}.{{ mgmtDns.adc001.octet }}", "{{ mgmtDns.adc002.cidr }}.{{ mgmtDns.adc002.octet }}" ]
        natMode: true
        namespaceNetworkCidr: "10.244.0.0/20"
        subnetPrefix: "28"
        serviceNetworkCidr: "10.96.0.0/23"
        ingressSegmentName: "tanzu-ingress-m01-seg01"
        ingressNetworkCidr: "x.x.x.x/24" #replace x.x.x.x/24 with correct value
        egressSegmentName: "tanzu-egress-m01-seg01"
        egressNetworkCidr: "x.x.x.x/24" #replace x.x.x.x/24 with correct value
#    workload: #for vcs002

  namespaces:
    - namespace: "napp-ns"
      cluster: "{{ locationCode }}-m01-cluster01"
      description: "NSX Application Platform namespace"
      access_list:
        ...
        - domain: "{{ searchDomain }}"
          role: "EDIT"
          subject: "role-{{ locationCode }}-g-networkadministrators"
          subject_type: "GROUP"
      storage_specs:
  ...
  vmclasses:
    - id: napp-class-advanced
      cpu_count: 16
      memory_MB: 65536

  tkg:
    - clusterName: tkg-napp
      resource_definition:
        ...
        spec:
          ...
          settings:
            ...
            network:
              proxy:
                httpProxy: http://{{ mgmtDns.pxy001.cidr }}.{{ mgmtDns.pxy001.octet }}:3128
                httpsProxy: http://{{ mgmtDns.pxy001.cidr }}.{{ mgmtDns.pxy001.octet }}:3128
...
1. Login to Ansible host VM and execute tanzuMgmtPrerequisites playbook:
    ...
    ```bash
    ansible-playbook tanzuMgmtPrerequisites.yml
    ```

    or, if you want to control separate tasks

    ```bash
    ansible-playbook tanzuMgmtPrerequisites.yml --tags=tag1,tag2
    ```
...
2. Login to Ansible host VM and execute tanzuMgmtSupervisor playbook:
    ...
    ```bash
    ansible-playbook tanzuMgmtSupervisor.yml
    ```

    or, if you want to control separate tasks

    ```bash
    ansible-playbook tanzuMgmtSupervisor.yml --tags=tag1,tag2
    ```
...
3. Login to Ansible host VM and execute tanzuEnableSupervisorMonitoring playbook:
    ...
    ```bash
    ansible-playbook tanzuEnableSupervisorMonitoring.yml -e VCS=vcs001
    ```

# Post deployment

## Create TKG cluster for NAPP (NSX Application Platform)

This step is OPTIONAL: Login to Ansible host VM and complete /opt/dhcConfig/group_vars/all/tanzuConfig.yml with required data: namespaces, vmclasses and tkg. You can find an example in this document: chapter "Deploy procedure - Adjust DHC variables".

Next execute createTkgNapp playbook:

```bash
ansible-playbook createTkgNapp.yml
```
...
NOTE: kubectl binary must be installed during tanzuMgmtSupervisor playbook execution.
