# Onboarding VMs to vRA On-Prem: Work Instruction

## Table of contents

- [Onboarding VMs to vRA On-Prem: Work Instruction](#onboarding-vms-to-vra-on-prem-work-instruction)
- [Changelog](#changelog)
- [Introduction](#introduction)
  ...
  - [Prerequisites](#prerequisites)
    - [Prepare wave sheet](#prepare-wave-sheet)
    - [Assign VM Owner to vRA On-Prem project](#assign-vm-owner-to-vra-on-prem-project)
    - [Verify Access to vRA On-Prem](#verify-access-to-vra-on-prem)
- [Automated Onboarding](#automated-onboarding)
  - [VRO Workflows](#vro-workflows)
    - [The Main Migration Workflow](#the-main-migration-workflow)
    - [Single VM Migration Workflow](#single-vm-migration-workflow)
    - [Create Migration Report Workflow](#create-migration-report-workflow)
  - [On-boarding Procedure](#on-boarding-procedure)
...
| blueprintName      | `Windows Server`                                                        | Yes        | Name of the blueprint                                                                  |
...
|  priorityGroup     | `na` (if `drProtected = no`) or `{srm-priority-number}` (if `drProtected = yes`)| Yes | SRM Priority group number or `na`                                                     |
|       rpo          | `na` (if `drProtected = no`) or `{srm-rpo}` (if `drProtected = yes`)     | Yes        | SRM RPO value for protection group or `na`                                             |
|   managedOs        | `yes` or `no`                                                           | Yes        | Indication if the VM is managed OS                                                     |
|   clusteredVm      | `na` (if `drProtected = no`) or `{srm-rpo}` (if `drProtected = yes`)     | Yes        | Indication if the VM is clustered VM (for WSFC clusters)                               |
...
In the CSV file `tenant` cannot be mixed, so it is allowed to onboard the VMs only into single vRA OnPrem tenant at once.
...
```csv
virtualmachinename,tenant,project,owner,blueprintName,backupPolicy,drProtected,protectionGroup,priorityGroup,rpo,managedOs,clusteredVm
testvm1,default-tenant,prd001,svc-gre32-vro001,Windows Server,daily_3w,no,na,na,na,no,na
testvm2,default-tenant,prd001,svc-gre32-vro001,RHEL Server,backupPolicy1,yes,PG_Group1,3,180,no,no
```
...
```csv
virtualmachinename,tenant,project,owner,blueprintName,backupPolicy,drProtected,protectionGroup,priorityGroup,rpo,managedOs,clusteredVm
testvm1,tenant1,prd001,svc-gre32-vro001,Windows Server,daily_3w,no,na,na,na,no,na
testvm2,tenant1,prd001,svc-gre32-vro001,RHEL Server,backupPolicy1,yes,PG_Group1,3,180,yes,no
```

### Assign VM Owner to vRA On-Prem project
...
1. **dhcOnboardMultipleVmsOnPrem** - the main workflow where the inputs are gathered from the user
2. **dhcOnboardSingleVmOnPrem** - the async workflow that is triggered from the main workflow for each VM to execute on-boarding
3. **dhcOnboardingReportOnPrem** - the workflow generating and sending a summary report, triggered by the main workflow at the end of the execution.

## VRO Workflows

### The Main Migration Workflow

Workflow name: **dhcOnboardMultipleVmsOnPrem**

![Figure 3](images/wiOnboardingVraOnpremAviva/mainWfOnprem.png)
...
3. Run the "dhcOnboardSingleVmOnPrem" Workflow asynchronously for each VM on the list.
4. Validate if all onboarding workflows are completed.
5. Gather errors (if any).
6. Trigger the "dhcOnboardingReportOnPrem" workflow.

### Single VM Migration Workflow

Workflow name: **dhcOnboardSingleVmOnPrem**

![Figure 4](images/wiOnboardingVraOnpremAviva/singleWfOnprem.png)
...
7. Fetch the Day1 Cloud Template ID based on name provided in CSV file.
8. Add Deployment to the on-boarding plan.
9. Assign Blueprint Template to the Deployment VM.
...
### Create Migration Report Workflow

Workflow name: **dhcOnboardingReportOnPrem**

![Figure 5](images/wiOnboardingVraOnpremAviva/reportWfOnprem.png)
...
Once all onboarding tasks are done for all VMs indicated in the CSV file, the summary report is created using **dhcOnboardingReportOnPrem** workflow.
