# Lifecycle Management - 1.8.2

## Table of Contents

- [Lifecycle Management - 1.8.2](#lifecycle-management---182)
  ...
  - [Update scenarios](#update-scenarios)
    - [From VCS 1.7.1 - full scope, all actions inlcuded for example vIDM de-clustering, Ubuntu update etc](#from-vcs-171---full-scope-all-actions-inlcuded-for-example-vidm-de-clustering-ubuntu-update-etc)
    - [From VCS 1.8.0 - without vIDM de-clustering, but with Ubuntu upgrade](#from-vcs-180---without-vidm-de-clustering-but-with-ubuntu-upgrade)
    - [From VCS 1.8.1 - without vIDM de-clustering and without Ubuntu upgrade](#from-vcs-181---without-vidm-de-clustering-and-without-ubuntu-upgrade)
...
| ---------- | -------- | --------------- | ---- | --------------------- |
| 26/06/2024 | VCS-13154 | Mariusz Stanek |      | Initial draft creation based on previous LCM versions |

## Introduction

This page describes Life Cycle Management of VCS components to version 1.8.2.

## Scope

The work instruction is intended to cover below update scenarios:

1. From VCS 1.7.1 - full scope, all actions included for example vIDM de-clustering, Ubuntu upgrade etc.
2. from VCS 1.8.0 - without vIDM de-clustering, but with Ubuntu.
3. from VCS 1.8.1 - without vIDM de-clustering and without Ubuntu upgrade.

## Related Documents

| Document |
| -------- |
| [VCS 1.8.2 - VCF upgrade to 4.5.2](dhcVcfUpgradeTo-4.5.2.md) |
| [Ubuntu 22 upgrade](https://github.com/GLB-CES-PrivateCloud/DHC-Documentation/blob/DHC-1.8/workInstructions/wiLifeCycleManagement-DHC1.8.1.md) |

## VCS Version matrix

[json1.8.2]: https://github.com/GLB-CES-PrivateCloud/DHC-Version-Matrix/blob/DHC-1.8.2/versionMatrix.json
...
From VCS 1.7 version matrix file structure has changed, contains new variables and includes components from the exact VCS version only (previous version are erased). See an example VCS 1.8.2 version fragment below:
...
For full LCM 1.8.2 components list refer to [versionMatrix.json][json1.8.2] file.
...
```bash
ansible-playbook upgradePlaybook.yml -e "componentCurrentVersion=dhcVersion1_7 componentNextVersion=dhcVersion1_8_2"
```
...
### From VCS 1.7.1 - full scope, all actions inlcuded for example vIDM de-clustering, Ubuntu update etc

Follow below WIs to upgrade from VCS 1.7.1 to 1.8.2:

1. [Ubuntu 22 upgrade](https://github.com/GLB-CES-PrivateCloud/DHC-Documentation/blob/DHC-1.8/workInstructions/wiLifeCycleManagement-DHC1.8.1.md)
2. [VCF 4.5.0 to VCF 4.5.2](dhcVcfUpgradeTo-4.5.2.md) (follow Steps list for version 1.7.1)

### From VCS 1.8.0 - without vIDM de-clustering, but with Ubuntu upgrade

Follow below WIs to upgrade from VCS 1.8.0 to 1.8.2:

1. [Ubuntu 22 upgrade](https://github.com/GLB-CES-PrivateCloud/DHC-Documentation/blob/DHC-1.8/workInstructions/wiLifeCycleManagement-DHC1.8.1.md)
2. [VCF 4.5.0 to VCF 4.5.2](dhcVcfUpgradeTo-4.5.2.md) (follow Steps list for version 1.8.x)

### From VCS 1.8.1 - without vIDM de-clustering and without Ubuntu upgrade

Follow below WIs to upgrade from VCS 1.8.1 to 1.8.2:

1. [VCF 4.5.0 to VCF 4.5.2](dhcVcfUpgradeTo-4.5.2.md) (follow Steps list for version 1.8.x)
...
```json
ansible-playbook upgradeDhcVersionInGroupVarsAll.yml -e "currentDhcVersion=dhcVersion1_8_2 nextDhcVersion=dhcVersion1_9"
```
...
> ```yaml
> # group_vars/all file
> componentCurrentVersion: dhcVersion1_8_2
